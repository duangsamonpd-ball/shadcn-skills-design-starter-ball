> # PROJECT OVERRIDE — read before anything below
> This file is a **vendored reference** for the `ux-ui-*` skills in this repo. It is **NOT** the
> project's `CLAUDE.md` and does not govern the app. Where it conflicts with the repo, the repo wins.
>
> **Design tokens — single source of truth is Figma → `shadcn-nextjs-design/DESIGN.md` +
> `src/app/globals.css`.** The `../_ux-ui-shared/tokens/*.json` here (DTCG / OKLCH) are **reference
> only**:
> - **Never** emit these token values into app code, and **never** create a competing `:root` /
>   `@theme` / CSS-variable layer. The repo already has one, sourced from Figma.
> - When a skill says "generate/extend/validate tokens" or "map tokens to a target", **read and defer
>   to `DESIGN.md` + `globals.css`**; treat the DTCG files only as format/structure examples.
> - Token sync direction is **Figma → DESIGN.md → globals.css** (see the repo `CLAUDE.md`). Do not
>   introduce Style Dictionary / Tokens Studio pipelines that would diverge from it.
>
> **Components:** install via the shadcn CLI into `src/components/ui/` per `shadcn-nextjs-design`;
> don't hand-roll what the registry provides. The verification scripts/gates below are usable, but the
> token/contrast values they check against come from the repo's Figma tokens, not the bundled JSON.
